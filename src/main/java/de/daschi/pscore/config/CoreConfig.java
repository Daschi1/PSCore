...
public class CoreConfig {
    ...
    public static void loadConfig() throws IOException, InvalidConfigurationException {
        CoreConfig.yamlFile = new YamlFile(new File("PSCore\\" + PSCore.getVersion() + "\\config.yml"));
        if (!CoreConfig.yamlFile.exists()) {
            final File[] files = new File("PSCore").listFiles();
            if (files != null && files.length - 2 >= 0) {
                final YamlFile oldConfig = new YamlFile(files[files.length - 2].getAbsolutePath() + "\\config.yml");
                if (oldConfig.exists()) {
                    oldConfig.copyTo(CoreConfig.yamlFile.getConfigurationFile().getAbsolutePath());
                }
            }
        }
